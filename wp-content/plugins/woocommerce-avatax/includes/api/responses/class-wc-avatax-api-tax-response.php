...
class WC_AvaTax_API_Tax_Response extends \WC_AvaTax_API_Response {
	 ...
	 * @return array
	 */
	public function get_cart_lines() {
	 ...
	 */
	public function get_origin_address( $line = null ) {
	 ...
	 */
	public function get_destination_address( $line = null ) {
	 ...
	 * @return bool
	 */
	public function is_tax_included() {
	...
	protected function build_rates( $raw_rates ) {
		...
		$has_landed_costs = isset( WC()->cart->avatax_has_landed_costs ) ? WC()->cart->avatax_has_landed_costs : $this->has_landed_costs();
		...
		if ( is_array( $raw_rates ) ) {
			...
			foreach ( $raw_rates as $raw_rate ) {
				...
				if ( $has_landed_costs ) {
					...
					$name = trim( "{$raw_rate->jurisdictionType} {$raw_rate->taxType}" );
				}

				// truncate name to full words between 30 and 35 characters
				$max_length = 35;
				$min_length = 30;
				...
				if ( strlen( $name ) > $max_length ) {
					...
					if ( strlen( $truncated_name ) < $min_length ) {
						...
						// too small, just truncate without caring about full words
	...
	/**
	 * Checks whether or not the rates include at least one landed cost.
	 *
	 * @since 1.10.0
	 *
	 * @return bool
	 */
	public function has_landed_costs() {
		...
		foreach ( $this->lines as $line ) {
			...
			foreach ( $line->details as $raw_rate ) {
				...
				if ( 'LandedCost' === $raw_rate->taxType ) {
					...
					$has_landed_costs = true;
					break 2;
				}
			}
		}

		return $has_landed_costs;
	}
