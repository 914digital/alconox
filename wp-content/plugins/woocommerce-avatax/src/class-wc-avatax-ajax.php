...
class WC_AvaTax_AJAX {
	...
	public function display_product_variation_code_fields( $loop, $variation_data, $variation ) {
		...
		include( wc_avatax()->get_plugin_path() . '/src/admin/views/html-field-product-variation-hts-code.php' );
	}
	...
	/**
	 * Toggles the Cross-Border sync state.
	 *
	 * @internal
	 *
	 * @since 1.13.0
	...
	public function toggle_cross_border_sync() {
		...
		if ( $toggled = wc_avatax()->get_landed_cost_handler()->is_subscribed_to_cross_border() ) {
			wc_avatax()->get_landed_cost_sync_handler()->toggle_syncing();
		}

		wp_send_json_success( [
			'toggled' => wc_bool_to_string( $toggled ),
		] );
	}
