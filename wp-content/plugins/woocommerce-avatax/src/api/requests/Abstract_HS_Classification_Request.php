...
abstract class Abstract_HS_Classification_Request extends WC_AvaTax_API_Request {
	...
	protected function get_classification_params() : array {
		...
		return [
			...
			'item'                 => [
				'companyId'   => $this->classification->get_item()->get_company_id(),
				'itemCode'    => $this->classification->get_item()->get_item_code(),
				'summary'     => $this->classification->get_item()->get_summary(),
				'description' => $this->classification->get_item()->get_description(),
				'itemGroup'   => $this->classification->get_item()->get_item_group(),
			],
		];
	}
