...
class WC_AvaTax_API_Tax_Request extends \WC_AvaTax_API_Request {
	...
	protected function prepare_product_lines( $items, $packages = array() ) {
		...
		foreach ( $items as $key => $item ) {
			...
			$hts_code = ( ! empty( $item['hts_code'] ) ) ? $item['hts_code'] : $this->get_hts_code( $product, $item['destination']['country'] );
			$hs_code  = ( ! empty( $item['hs_code'] ) ) ? $item['hs_code'] : $this->get_hs_code( $product, $item['destination']['country'] );
	...
	/**
	 * Gets the proper HS code to be used in the request line.
	 *
	 * @since 1.13.0
	 *
	 * @param WC_Product $product a WC product
	 * @param string $country the destination country
	 * @return string the HS code if available
	 */
	protected function get_hs_code( WC_Product $product, string $country = '' ) : string {

		return wc_avatax()->get_landed_cost_handler()->get_hs_code( $product, $country );
	}


	/**
	 * Gets the proper HTS code to be used in the request line.
	 *
	 * @since 1.13.0
	 *
	 * @param WC_Product $product a WC product
	 * @param string $country the destination country
	 * @return string the HTS/HS code if available
	 */
	protected function get_hts_code( WC_Product $product, string $country = '' ) : string {

		return wc_avatax()->get_landed_cost_handler()->get_hts_code( $product, $country );
	}
	...
	protected function prepare_line( $item ) {
		...
		// add landed cost
		if ( ( $item['hsCode'] || $item['HTSCode'] ) && wc_avatax()->get_landed_cost_handler()->is_available() ) {

			$line['hsCode'] = ! empty( $item['hsCode'] ) ? $item['hsCode'] : $item['HTSCode'];
