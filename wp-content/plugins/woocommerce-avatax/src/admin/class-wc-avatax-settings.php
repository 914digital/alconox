...
class WC_AvaTax_Settings {
	...
	private function get_cross_border_product_sync_description() : string {
		...
		if ( ! $this->is_api_environment_production() ) {
			$sync_error_items .= '<li>' . __( 'Product sync is unavailable for sandbox accounts.', 'woocommerce-avatax' ) . '</li>';
		}
		...
		if ( ! $this->get_plugin()->get_landed_cost_sync_handler()->supports_background_processing() ) {
			$sync_error_items .= '<li>' . __( 'This site does not support background processing, which is required for cross-border catalog sync. Please ask your host to enable background processing for your site and try again.', 'woocommerce-avatax' ) . '</li>';
		}
	...
	public function handle_countries_sync_update( $old_countries, $new_countries ) {
		...
		if ( $sync_handler->is_syncing_active() && ! empty( array_diff( $new_countries, $old_countries ) ) ) {
			$sync_handler->enqueue_all_products();
		}
	}
	...
	public function is_cross_border_product_sync_connection_allowed() : bool {
		...
		$connection_allowed =
			...
			&& $this->is_api_environment_production()
			&& $this->get_plugin()->get_landed_cost_handler()->has_countries_for_product_sync()
			&& $this->get_plugin()->get_landed_cost_sync_handler()->supports_background_processing();
