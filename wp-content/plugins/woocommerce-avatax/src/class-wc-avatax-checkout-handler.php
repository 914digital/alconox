...
class WC_AvaTax_Checkout_Handler {
	...
	public function calculate_taxes( $cart ) {
		...
		try {
			...
			// (has to run before setting avatax_rates, because it is used by it)
	 ...
	 */
	protected function set_fee_taxes( WC_Cart $cart, $lines ) {
		...
		foreach ( $lines as $line ) {
			...
			$found = false;
			foreach ( $cart_fee_ids as $cart_fee_id ) {

				if ( hash_equals( wp_hash( $cart_fee_id ), $line_id ) ) {
			...
			// generated by Avalara that needs to be added
			if ( ! $found ) {
			...
			foreach ( $cart_fee_ids as $cart_fee_key => $cart_fee_id ) {
				...
				// search for either hashed or non hashed ID (fee generated by Avalara)
				if ( $cart_fee_id === $line_id || hash_equals( wp_hash( $cart_fee_id ), $line_id ) ) {

					$fee_key = $cart_fee_key;
					break;
				}
			}
			...
			$tax_data  = isset( $cart_fees[ $fee_key ]->tax_data ) ? $cart_fees[ $fee_key ]->tax_data : array();
