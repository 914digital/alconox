 ...
 */
		add_settings_section(
			...
			esc_html__('WP Login Captcha Settings', 'recaptcha_verification'),                  // Title to be displayed on the administration page.
		...
		if (! function_exists('wp_login_captcha_check_setting') ) {
			function wp_login_captcha_check_setting() {
				?>
		<input type="checkbox" name="wp_login_captcha_check" value="1"<?php checked(get_option('wp_login_captcha_check'), '1'); ?>><label><?php echo esc_html__(' Eables captcha for WordPress login page', 'recaptcha_verification'); ?></label><br><br>
				<?php
			}
		}
		if (! function_exists('wp_login_captcha_title_set') ) {
			function wp_login_captcha_title_set() {
				?>
		<input type="text" name="wp_login_captcha_title" value="<?php echo esc_attr(get_option('wp_login_captcha_title')); ?>"><label><?php echo esc_html__(' Field text for captcha in WordPress login page', 'recaptcha_verification'); ?></label><br><br>
				<?php
			}
		}
		if (! function_exists('wp_captcha_themes_settings') ) {
			function wp_captcha_themes_settings() {
				?>
		<input type="radio" name="wp_login_captcha_theme_fields" value="1"<?php checked(get_option('wp_login_captcha_theme_fields'), '1'); ?> checked  >
		...
		<input type="radio" name="wp_login_captcha_theme_fields" value="0"<?php checked(get_option('wp_login_captcha_theme_fields'), '0'); ?>  >
		...
		if (! function_exists('wp_login_captcha_size_setting') ) {
			function wp_login_captcha_size_setting() {
				?>
		<input type="radio" name="wp_login_captcha_size_radio" value="1"<?php checked(get_option('wp_login_captcha_size_radio'), '1'); ?>  checked  >
		...
		<input type="radio" name="wp_login_captcha_size_radio" value="0"<?php checked(get_option('wp_login_captcha_size_radio'), '0'); ?>  >
