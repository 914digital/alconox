...
class AgileStoreLocator_Admin {
  ...
  private function _get_custom_fields() {
	...
	public function add_new_store() {
		...
		if(isset($form_data['ddl_f_states'])) {

			$form_data['states'] =  (is_array($form_data['ddl_f_states']))? implode(',', $form_data['ddl_f_states']): $form_data['ddl_f_states'];
			unset($form_data['ddl_f_states']);
		}



		if(isset($form_data['ddl_f_countries'])) {

			$form_data['countries'] = (is_array($form_data['ddl_f_countries']))? implode(',', $form_data['ddl_f_countries']): $form_data['ddl_f_countries'];
			unset($form_data['ddl_f_countries']);
		}
		...
		//	Custom Field
		if(isset($_REQUEST['asl-custom']) && $_REQUEST['asl-custom']) {
	...
	public function delete_store() {
		...
		if($multiple) {
			...
			$delete_sql 	 = "DELETE FROM ".ASL_PREFIX."stores WHERE id IN (".sanitize_text_field($item_ids).")";
		}
		else {

			$store_id 		 = $_REQUEST['store_id'];
			$delete_sql 	 = "DELETE FROM ".ASL_PREFIX."stores WHERE id = ".sanitize_text_field($store_id);
		}
	...
	public function store_status() {
		...
		$item_ids 		 = implode(",",$_POST['item_ids']);
		$update_sql 	 = "UPDATE ".ASL_PREFIX."stores SET is_disabled = {$status} WHERE id IN (".sanitize_text_field($item_ids).")";
	...
	public function duplicate_store() {
		...
		$result = $wpdb->get_results("SELECT * FROM ".ASL_PREFIX."stores WHERE id = ".sanitize_text_field($store_id));		
		...
		if($result && $result[0]) {
			...
			if($wpdb->insert( ASL_PREFIX.'stores', $result)){
				...
				$s_categories = $wpdb->get_results("SELECT * FROM ".ASL_PREFIX."stores_categories WHERE store_id = ".sanitize_text_field($store_id));
	 ...
	 * @return [type] [description]
	 */
	public function admin_manage_countries() {

		// add scripts
    $this->_enqueue_scripts();

		include ASL_PLUGIN_PATH.'admin/partials/countries.php';
	}
	 ...
	 * @return [type] [description]
	 */
	public function admin_manage_states() {

		// add scripts
    $this->_enqueue_scripts();

		include ASL_PLUGIN_PATH.'admin/partials/states.php';
	}
	...
	public function delete_attribute() {
		...
		if($multiple) {
			...
			$item_ids 		 = implode(",", sanitize_text_field($_POST['item_ids']));
	...
	public function delete_category() {
		...
		if($multiple) {
			...
			$item_ids 		 = implode(",",$_POST['item_ids']);
			$delete_sql 	 = "DELETE FROM ".ASL_PREFIX."categories WHERE id IN (".sanitize_text_field($item_ids).")";
			$cResults 		 = $wpdb->get_results("SELECT * FROM ".ASL_PREFIX."categories WHERE id IN (".sanitize_text_field($item_ids).")");
		}
		else {

			$category_id 	 = $_REQUEST['category_id'];
			$delete_sql 	 = "DELETE FROM ".ASL_PREFIX."categories WHERE id = ".sanitize_text_field($category_id);
			$cResults 		 = $wpdb->get_results("SELECT * FROM ".ASL_PREFIX."categories WHERE id = ".sanitize_text_field($category_id) );
		}
	...
	public function update_category() {
		...
		if($data['action'] == "notsame") {
			...
			$res = $wpdb->get_results( "SELECT * FROM ".ASL_PREFIX."categories WHERE id = ".sanitize_text_field($data['category_id']));
	...
	public function get_category_by_id() {
		...
		$response->list = $wpdb->get_results( "SELECT * FROM ".ASL_PREFIX."categories WHERE id = ".sanitize_text_field($store_id));
	...
	public function delete_marker() {
		...
		if($multiple) {
			...
			$item_ids 		 = implode(",",$_POST['item_ids']);
			$delete_sql 	 = "DELETE FROM ".ASL_PREFIX."markers WHERE id IN (".sanitize_text_field($item_ids).")";
			$mResults 		 = $wpdb->get_results("SELECT * FROM ".ASL_PREFIX."markers WHERE id IN (".sanitize_text_field($item_ids).")");
		}
		else {

			$item_id 		 = $_REQUEST['marker_id'];
			$delete_sql 	 = "DELETE FROM ".ASL_PREFIX."markers WHERE id = ".sanitize_text_field($item_id);
			$mResults 		 = $wpdb->get_results("SELECT * FROM ".ASL_PREFIX."markers WHERE id = ".sanitize_text_field($item_id) );
		}
	...
	public function update_marker() {
		...
		if($data['action'] == "notsame") {
			...
			$res = $wpdb->get_results( "SELECT * FROM ".ASL_PREFIX."markers WHERE id = ".sanitize_text_field($data['marker_id']));
	...
	public function get_marker_by_id() {
		...
		$response->list = $wpdb->get_results( "SELECT * FROM ".ASL_PREFIX."markers WHERE id = ".sanitize_text_field($store_id));
	...
	public function delete_logo() {
		...
		if($multiple) {
			...
			$item_ids 		 = implode(",",$_POST['item_ids']);
			$delete_sql 	 = "DELETE FROM ".ASL_PREFIX."storelogos WHERE id IN (".sanitize_text_field($item_ids).")";
			$mResults 		 = $wpdb->get_results("SELECT * FROM ".ASL_PREFIX."storelogos WHERE id IN (".sanitize_text_field($item_ids).")");
		}
		else {

			$item_id 		 = $_REQUEST['logo_id'];
			$delete_sql 	 = "DELETE FROM ".ASL_PREFIX."storelogos WHERE id = ".sanitize_text_field($item_id);
			$mResults 		 = $wpdb->get_results("SELECT * FROM ".ASL_PREFIX."storelogos WHERE id = ".sanitize_text_field($item_id) );
		}
	...
	public function update_logo() {
		...
		if($data['action'] == "notsame") {
			...
			$res = $wpdb->get_results( "SELECT * FROM ".ASL_PREFIX."storelogos WHERE id = ".sanitize_text_field($data['logo_id']));
	...
	public function get_logo_by_id() {
		...
		$response->list = $wpdb->get_results( "SELECT * FROM ".ASL_PREFIX."storelogos WHERE id = ".sanitize_text_field($store_id));
		...
		*/
		...
		foreach($rows as $t) {
			...
			if(isset($t['logo_name']) && $t['logo_name'] != '') {
				...
				$logoresult = $wpdb->get_results("SELECT * FROM ".ASL_PREFIX."storelogos WHERE name = '".sanitize_text_field($t['logo_name'])."'" );
			...
			if($t['countries'] != '') {
				...
				$countries = explode("|", $t['countries']);

				foreach ($countries as $f_country) {
					
					$f_country 					= trim($f_country);
					$f_country_in_db 		= $wpdb->get_results("SELECT * FROM ".ASL_PREFIX."filter_countries WHERE name = '".sanitize_text_field($f_country)."'" );
					
					
					//	Get the ID
					if(count($f_country_in_db) > 0) {

						$countries_ids[] = $f_country_in_db[0]->id;

					}
					//	Add it and Save ID
					else {

						$wpdb->insert(ASL_PREFIX.'filter_countries', 
					 	array(
					 			'name' => $f_country,			 		
								'is_active'		=> 1
					 		),
					 	...
					 	$response->summary[] = 'Row: '.$index.', Country Created: '.$f_country;

					 	$countries_ids[] = $wpdb->insert_id;
					}
				}
			}
			...
			if($t['states'] != '') {
			  ...
			  $states = explode("|", $t['states']);

			  foreach ($states as $f_state) {
			    
			    $f_state          = trim($f_state);
			    $f_state_in_db    = $wpdb->get_results("SELECT * FROM ".ASL_PREFIX."filter_states WHERE name = '".sanitize_text_field($f_state)."'" );
			    
			    
			    //  Get the ID
			    if(count($f_state_in_db) > 0) {

			      $states_ids[] = $f_state_in_db[0]->id;

			    }
			    //  Add it and Save ID
			    else {

			      $wpdb->insert(ASL_PREFIX.'filter_states', 
			      array(
			          'name' => $f_state,         
			          'is_active'   => 1
			        ),
			      ...
			      $response->summary[] = 'Row: '.$index.', State Created: '.$f_state;

			      $states_ids[] = $wpdb->insert_id;
			    }
			  }
			}
			...
			if($create_category && $t['categories'] != '') {
				...
				foreach ($categorys as $_cat) {
					...
					try {
						...
						$category_count = $wpdb->get_results("SELECT count(*) AS `count` FROM ".ASL_PREFIX."categories WHERE category_name = '".sanitize_text_field($_cat)."';");						
			...
			if($t['title'] != '') {
				...
				if($store_id && $t['categories'] != '') {
					...
					if($is_update) {
						$wpdb->query("DELETE FROM ".ASL_PREFIX."stores_categories WHERE store_id = ".sanitize_text_field($store_id));						
					}

					foreach ($categorys as $category) {
						
						$category   = trim($category);
						$categoryid = $wpdb->get_results("SELECT * FROM ".ASL_PREFIX."categories WHERE category_name = '".sanitize_text_field($category)."'" );
  ...
  public function export_store() {
    ...
    //	Header titles
    $headers = ['title', 'description', 'street', 'city', 'state', 'postal_code', 'country', 'lat', 'lng', 'phone', 'fax', 'email', 'website', 'is_disabled', 'logo', 'categories', 'marker_id', 'description_2', 'open_hours', 'order', 'countries', 'states'];
    ...
    foreach ($stores as $value) {
      ...
      $logo_name 	= $wpdb->get_results("SELECT * FROM ".ASL_PREFIX."storelogos WHERE id = ".sanitize_text_field($value->logo_id) ); 
      ...
      $categories = $wpdb->get_results("SELECT * FROM ".ASL_PREFIX."categories RIGHT JOIN ".ASL_PREFIX."stores_categories ON  
        ".ASL_PREFIX."categories.id  = ".ASL_PREFIX."stores_categories.category_id WHERE ".ASL_PREFIX."stores_categories.store_id = ".sanitize_text_field($value->id));
      ...
      ///////////////
			...
			if($value->countries) {
				...
				$countries_in_db 		= $wpdb->get_results("SELECT GROUP_CONCAT(`name` SEPARATOR '|') AS b FROM ".ASL_PREFIX."filter_countries WHERE id IN (".$value->countries.")");

				if(count($countries_in_db) > 0) {
					$countries = $countries_in_db[0]->b;
				}
			}
      ...
      if($value->states) {
        ...
        $states_in_db    = $wpdb->get_results("SELECT GROUP_CONCAT(`name` SEPARATOR '|') AS b FROM ".ASL_PREFIX."filter_states WHERE id IN (".$value->states.")");

        if(count($states_in_db) > 0) {
          $states = $states_in_db[0]->b;
        }
      }
    ...
    $this->_enqueue_scripts();
		...
		$filter_countries 	 = $wpdb->get_results( "SELECT * FROM ".ASL_PREFIX."filter_countries");
		$filter_states 	 		 = $wpdb->get_results( "SELECT * FROM ".ASL_PREFIX."filter_states");
		...
		//	Store Data
		$store  = $wpdb->get_results("SELECT * FROM ".ASL_PREFIX."stores WHERE id = ".sanitize_text_field($store_id));		
		

		$storecategory = $wpdb->get_results("SELECT * FROM ".ASL_PREFIX."stores_categories WHERE store_id = ".sanitize_text_field($store_id));
		...
		$store_countries 	= explode(',', $store->countries);
		$store_states 		= explode(',', $store->states);


		$storelogo = $wpdb->get_results("SELECT * FROM ".ASL_PREFIX."storelogos WHERE id = ".sanitize_text_field($store->logo_id));
