...
class AgileStoreLocator_Activator {
	...
	public static function add_basic_tables() {
		...
		//////////*Attributes*/////////
		$sql = "CREATE TABLE IF NOT EXISTS `{$prefix}filter_countries` (
			  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
		...
		$c = $wpdb->get_results("SELECT count(*) AS 'count' FROM {$prefix}filter_countries");
		if($c[0]->count <= 0) {
			
			$sql =  "INSERT INTO `{$prefix}filter_countries` (`name`) VALUES ('United States');";
			dbDelta( $sql );
		}


		$sql = "CREATE TABLE IF NOT EXISTS `{$prefix}filter_states` (
			  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
			  `name` varchar(255) DEFAULT NULL,
			  `is_active` tinyint(4) NULL DEFAULT 1,
			  `created_on` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
		...
		$sql 	= "SELECT count(*) as c FROM information_schema.COLUMNS WHERE TABLE_NAME = '{$prefix}stores' AND COLUMN_NAME = 'countries' AND TABLE_SCHEMA = '{$database}'";
		$result = $wpdb->get_results($sql);
		
		if($result[0]->c == 0) {

			$wpdb->query("ALTER TABLE {$prefix}stores ADD COLUMN `countries` text;");
			$wpdb->query("ALTER TABLE {$prefix}stores ADD COLUMN `states` text;");
