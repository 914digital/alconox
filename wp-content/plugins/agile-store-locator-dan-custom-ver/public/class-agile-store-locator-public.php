  ...
  */
  ...
  public function frontendStoreLocator($atts) {
    ...
    */
    ...
    $tmp_data = $wpdb->get_results("SELECT id, name FROM ".ASL_PREFIX."filter_countries ORDER BY name = 'United States' DESC, name ASC");

    $all_attributes->countries = array();

    foreach($tmp_data as $r) {
      $all_attributes->countries[] = $r;
    }

    
    $tmp_data = $wpdb->get_results("SELECT id, name FROM ".ASL_PREFIX."filter_states ORDER BY name ASC");

    $all_attributes->states = array();

    foreach($tmp_data as $r) {
      $all_attributes->states[$r->id] = $r;
    }
  ...
  public function load_stores() {
    ...
    */
    ...
    $query   = "SELECT s.`id`, `title`,  `description`, `street`,  `city`,  `state`, `postal_code`, {$country_field} `lat`,`lng`,`phone`,  `fax`,`email`,`website`,`logo_id`,{$ASL_PREFIX}storelogos.`path`,`marker_id`,`description_2`,`open_hours`, `ordr`,`countries`,`states`, `custom`,
